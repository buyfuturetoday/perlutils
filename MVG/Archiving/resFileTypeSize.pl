...
resFileTypeSize - Results of File Type Analysis (Size)
...
=head1 SYNOPSIS
 ...
 resFileTypeSize.pl [-t] [-l log_dir]

 resFileTypeSize -h			Usage
 resFileTypeSize -h 1		Usage and description of the options
 resFileTypeSize -h 2		All documentation
...
$query = "SELECT filetype, sum(files) as totalfiles, sum(sizeused) as totalsizeused, sum(sizealloc) as totalsizealloc FROM $resulttable GROUP BY filetype ORDER BY totalsizeused DESC LIMIT $limit";
logging($query);
...
print HTML "<table bgcolor='$bgcolor' border cellpadding=2>\n";
print HTML "<tr><th>Filetype<th>Files<th>Size Used (MB)<th>Size Allocated (MB)</tr>\n";
$query = "SELECT filetype, sum(files) as totalfiles, sum(sizeused) as totalsizeused, sum(sizealloc) as totalsizealloc FROM $resulttable WHERE server = '$server' AND drive = '$drive' GROUP BY filetype ORDER BY totalsizeused DESC LIMIT $limit";
logging($query);
$sth = $dbres->prepare($query);
$rv = $sth->execute();
if (not(defined $rv)) {
