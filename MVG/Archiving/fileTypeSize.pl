...
fileTypeSize - Top File Types according to size.
...
=head1 SYNOPSIS
 ...
 fileTypeSize.pl [-t] [-l log_dir] [-a | -n YYYY-MM-DD]

 fileTypeSize -h	   Usage
 fileTypeSize -h 1	   Usage and description of the options
 fileTypeSize -h 2	   All documentation
...
sub handle_query($) {
	...
	my $query = "SELECT FileType, count(*) as totalfiles, sum(sizeusedmb) as sizeused, sum(allocatedmb) as allocsize FROM $tablename GROUP BY FileType ORDER BY sizeused DESC LIMIT 20";
	logging($query);
