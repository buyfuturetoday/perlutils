...
getRaidGroupFragment - Collect RaidGroupFragment information for Customer
...
This application will get the RaidGroupFragment Information for all raidgroups from all boxes in the customer environment. Therefore the boxes for the customer will be collected first. For each box all RaidGroup information will then be collected. 

For each Raid Group the raid group fragments will be extracted and collected in the Raidgroup Fragment table. Raidgroup fragments should come from raidgroup types r5 and r1_0, not from types Unbound or hot_spare.

=head1 SYNOPSIS

 getRaidGroupFragment.pl [-t] [-l log_dir] 

 getRaidGroupFragment -h	   Usage
 getRaidGroupFragment -h 1	   Usage and description of the options
 getRaidGroupFragment -h 2	   All documentation
...
my ($logdir, $dbhsource, $dbhtarget, @boxes, @raidgroups);
my $username = "root";
...
=head2 Add to RaidGroupFragment

This procedure will add the RaidgroupFragment information to the DWH RaidgroupFragment table.

=cut

sub add2rgfrag {
	my($rgfrag, $name, $number, $kb_log, $kb_raw, $wwlunid, $rg, $vol) = @_;
	my $query = "INSERT INTO rgfrag (rgfrag,name,number,kb_log,kb_raw,wwlunid,rg,vol) 
				 VALUES ('$rgfrag', '$name', '$number', '$kb_log', '$kb_raw', '$wwlunid', '$rg', '$vol')";
...
# Now collect all Raidgroup fragments per Raid Group
foreach my $rg (@raidgroups) {
	my $query = "SELECT rgfrag, kb_log, kb_raw, name, number, wwlunid, vol
	...
	while (my $ref = $sth->fetchrow_hashref) {
		...
		my $vol = $ref->{vol};
		add2rgfrag($rgfrag, $name, $number, $kb_log, $kb_raw, $wwlunid, $rg, $vol);
	}
}
$sth->finish();
