...
fileTypeNumber - Top File Types according to number of files.
...
This application will fill the table with file type information, ordered by number of files.

=head1 SYNOPSIS

 fileTypeNumber.pl [-t] [-l log_dir] [-a | -n YYYY-MM-DD]

 fileTypeNumber -h		   Usage
 fileTypeNumber -h 1	   Usage and description of the options
 fileTypeNumber -h 2	   All documentation
...
sub handle_query($) {
	...
	my $query = "SELECT FileType, count(*) as totalfiles, sum(sizeusedmb) as sizeused, sum(allocatedmb) as allocsize FROM $tablename GROUP BY FileType ORDER BY totalfiles DESC LIMIT 20";
	logging($query);
	...
	while (my $ref = $sth->fetchrow_hashref) {
		...
		add2resulttable($server,$drive,$filetype,$totalfiles,$sizeused,$allocsize);
	}
	$sth->finish();
}
...
sub add2resulttable() {
	my($server,$drive,$filetype,$files,$sizeused,$sizealloc) = @_;
	my $query = "INSERT INTO $resulttable 
				 (DateID,Server,Drive,FileType,Files,SizeUsed,SizeAlloc) VALUES
				 ('$dateid','$server','$drive','$filetype','$files','$sizeused','$sizealloc')";
