...
version 1.2 11 October 2006 DV

=over 4

=item *

Handle warning thresholds as successful replies in charts.

=back

version 1.1 4 October 2006 AC
...
my $interval = 2;		    # Interval (hours)
my $cnt = 0;
my $outdir = "d:/em/data/reps/std";
...
sub extract_data($) {
    ...
    # Find max from all successful or warning response times
    # 15% from max value is negative value.
    # If (0 > Invalid Response Time > -15), then set Invalid Response Time to -15 to guarantee visibility.
	...
	if ($invalid_resp_time > -15) {
	    $invalid_resp_time = -15;
	}
    }
