...
resLargeFiles - Results of Large Files Analysis
...
This application will create the results from the Large File analysis.

=head1 SYNOPSIS

 resLargeFiles.pl [-t] [-l log_dir]

 resLargeFiles -h		Usage
 resLargeFiles -h 1		Usage and description of the options
 resLargeFiles -h 2		All documentation
...
print XL "Filename; Path; Size Used (MB); Size Allocated (MB); Created; Modified; Accessed; Created Days; Modified Days; Accessed Days; File Type; Server; Drive\n";
my $query = "SELECT filename, path, sizeusedmb, allocatedmb, created, modified, accessed, createddays, modifieddays, accesseddays, filetype, server, drive FROM $resulttable";
logging($query);
...
while (my $ref = $sth->fetchrow_hashref) {
	...
	my $path = $ref->{path};
	my $sizeusedmb = $ref->{sizeusedmb};
	my $allocatedmb = $ref->{allocatedmb};
	my $created = $ref->{created};
	my $modified = $ref->{modified};
	my $accessed = $ref->{accessed};
	my $createddays = $ref->{createddays};
	my $modifieddays = $ref->{modifieddays};
	my $accesseddays = $ref->{accesseddays};
	my $filetype = $ref->{filetype};
	my $server = $ref->{server};
	my $drive = $ref->{drive};
	print XL "filename, path, sizeusedmb, allocatedmb, created, modified, accessed, createddays, modifieddays, accesseddays, filetype, server, drive\n";
}
$sth->finish();
# Close csv result file
close XL;

# Overall largest files
...
print HTML "<tr><th>Filename<th>Size Used (MB)<th>Size Allocated (MB)<th>Created<th>Modified<th>Accessed<th>Server<th>Drive</tr>\n";
$query = "SELECT filename, sizeusedmb, allocatedmb, created, modified, accessed, server, drive FROM $resulttable ORDER BY sizeusedmb DESC LIMIT $limit";
logging($query);
...
while (my $ref = $sth->fetchrow_hashref) {
	...
	my $drive = $ref->{drive};
	# Format numbers
	my $sizeusedmb_f = $nf->format_number($sizeusedmb,2,2);
	my $allocatedmb_f = $nf->format_number($allocatedmb,2,2);
	print HTML "<tr><td align='right'>$filename<td align='right'>$sizeusedmb_f<td align='right'>$allocatedmb_f<td align='right'>$created<td align='right'>$modified<td align='right'>$accessed<td align='right'>$server<td align='right'>$drive</tr>\n";
}
print HTML "</table><p>\n";
$sth->finish();
close HTML;
...
print HTML "<table bgcolor='$bgcolor' border cellpadding=2>\n";
print HTML "<tr><th>Filename<th>Size Used (MB)<th>Size Allocated (MB)<th>Created<th>Modified<th>Accessed</tr>\n";
$query = "SELECT filename, sizeusedmb, allocatedmb, created, modified, accessed FROM $resulttable WHERE server = '$server' and drive = '$drive' ORDER BY sizeusedmb DESC LIMIT $limit";
logging($query);
my $sth = $dbres->prepare($query);
my $rv = $sth->execute();
if (not(defined $rv)) {
...
while (my $ref = $sth->fetchrow_hashref) {
	...
	my $sizeusedmb_f = $nf->format_number($sizeusedmb,2,2);
	my $allocatedmb_f = $nf->format_number($allocatedmb,2,2);
	print HTML "<tr><td align='right'>$filename<td align='right'>$sizeusedmb_f<td align='right'>$allocatedmb_f<td align='right'>$created<td align='right'>$modified<td align='right'>$accessed</tr>\n";
}
print HTML "</table><p>\n";
$sth->finish();
close HTML;
