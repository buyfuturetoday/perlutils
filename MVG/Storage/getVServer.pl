...
getVServer - Collect Virtual Server information for Customer
...
This application will get the Virtual Server information related to the shared storage from the customer.

=head1 SYNOPSIS

 getVServer.pl [-t] [-l log_dir] 

 getVServer -h	 	   Usage
 getVServer -h 1	   Usage and description of the options
 getVServer -h 2	   All documentation
...
sub add2vserver {
	my($servergroup,$vserver,$zone,$reference,$description) = @_;
	my $query = "INSERT INTO vserver (servergroup,vserver,zone,reference,description) 
				 VALUES ('$servergroup','$vserver','$zone','$reference','$description')";
...
foreach my $serverg (@servergroups) {
	my $query = "SELECT vserver,zone,ref,descr FROM t_vserver WHERE serverg='$serverg'";
	my $sth = $dbhsource->prepare($query);
	my $rv = $sth->execute();
	if (not(defined $rv)) {
		error("Could not execute query $query, Error: ".$sth->errstr);
	...
	while (my $ref = $sth->fetchrow_hashref) {
		...
		my $descr = $ref->{descr} || "";
		add2vserver($serverg,$vserver,$zone,$reference,$descr);
	}
	$sth->finish();
}
