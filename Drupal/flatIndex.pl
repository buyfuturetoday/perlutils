...
flatIndex - This scipt creates the index files associated with the flat books.
...
After the script flatBooks created a flat version of Drupal book Collection, this script will create the index files.

=head1 SYNOPSIS

 flatIndex.pl [-t] [-l log_dir] [-d databasename] -s dumpdirectorystore

 flatIndex -h	   Usage
 flatIndex -h 1   Usage and description of the options
 flatIndex -h 2   All documentation
...
# Open Book Collection Index File
my ($filehandle);
my $indexfile = "$bookdir/index.html";
my $openres = open($filehandle, ">$indexfile");
if (not defined $openres) {
	error("Could not open $indexfile for writing, exiting...");
	exit_application(1);
}
print $filehandle "<h1>$databasename</h1>\n";
print $filehandle "<ul>\n";

# Find Book Node
my $query = "SELECT nid, ml.mlid as mlid
				FROM menu_links ml, book b
				WHERE module='book' and depth=1
					AND ml.mlid = b.mlid
				ORDER BY link_title";
my $sth = $dbh->prepare($query);
my $rv = $sth->execute();
...
while (my $ref = $sth->fetchrow_hashref()) {
	my $mlid = $ref->{mlid};
	my $nid = $ref->{nid};
	# Add entry in Collection index file
	my ($title, undef) = getnode($nid);
	print $filehandle "\t<li><a href='./$title/index.html'>$title</a>\n";
	# Handle the book
