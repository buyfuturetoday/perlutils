...
WRSExtractDevFailed.pl - Script to extract Device data from the CAUM4WS table for failed transactions.
...
sub extract_data() {
    my $sql = "SELECT TOP $top_count WRMGroup.Name AS 'Group', WRMIdentifier.Name AS Identifier, WRM.TimeStart, 
	       ...
	       WHERE (WRM.GroupID = $identifier) and 
		     (WRM.NewSumStatID <> 2)
...
# Open Output file
my $outfile = $rep_dir."/Group_Failed_$identifier.html";
...
print OutFile "<h3>Last $top_count Failed Connections (Extracted at $datetime)</h3>\n";
