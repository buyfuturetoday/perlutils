...
flatBooks - Dump all Drupal Books into flat files for data backup and dummy browser export.

=head1 VERSION HISTORY

version 1.0 06 March 2010 DV
...
=head1 SYNOPSIS
 ...
 flatBooks.pl [-t] [-l log_dir] [-d databasename] -s dumpdirectorystore -Y

 flatBooks -h	   Usage
 flatBooks -h 1   Usage and description of the options
 flatBooks -h 2   All documentation
...
if (-d "$bookdir") {
	...
	if ((defined $deleteflag) && ($deleteflag eq "Yes")) {
		rmtree("$bookdir", {error => \my $err});
		handle_patherror("delete $bookdir", $err);
	} else {
		error("Book directory $bookdir still exists, specify delete flag -Y!");
		exit_application(1);
	}
}
...
my $query = "SELECT nid, ml.mlid as mlid, link_title
				FROM menu_links ml, book b
