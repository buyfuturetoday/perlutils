...
my ($dbconn_site,$dbconn_loc,$dbconn_gencomp,$omschrijving,$gc_count);
my (%componenten,$site_code,$gemeente,$naam);
my $outputfile = "c:/temp/ComponentenPerSite.csv";
...
sub process_locations {
	...
	my $locid = $loc_record{locid};
	my @typeids;
	# For each location, find the different components
	$sqlquery="SELECT distinct(gctypeid) FROM $gencomp_table WHERE locid=$locid";
	SQLquery($dbconn_gencomp, $sqlquery);
	# Add each component to an array for further processing
	# if the component is from the "server" class
	while ($dbconn_gencomp->FetchRow()) {
	    my %gencomp_record=$dbconn_gencomp->DataHash();
	    my $gctypeid=$gencomp_record{gctypeid};
	    if (exists($componenten{$gctypeid})) {
		push (@typeids,$gctypeid);
	    }
	}
	# For each type id, count number of occurences
	while (my $gctypeid=pop(@typeids)) {
	    # Count the number of occurences
	    $sqlquery="SELECT count(*) FROM $gencomp_table
			   WHERE locid=$locid AND gctypeid=$gctypeid";
	    SQLquery($dbconn_gencomp,$sqlquery);
	    if ($dbconn_gencomp->FetchRow()) {
	        my %gencomp_record=$dbconn_gencomp->DataHash();
	        (undef,$gc_count)=%gencomp_record;
	    } else {
	        error("gctypeid $gctypeid disappeared for locid $locid in $gencomp_table");
	    }
	    # Add the number to the total number for the site
	    if (exists($comps_total{$gctypeid})) {
		$comps_total{$gctypeid}=$comps_total{$gctypeid}+$gc_count;
	    } else {
		$comps_total{$gctypeid}=$gc_count;
	    }
	}
    }
    # Print the Summary per site
    while(my($gctypeid,$gc_count)=each %comps_total) {
        print OUTPUTFILE "$site_code;$gemeente;$naam;$componenten{$gctypeid};$gc_count\n";
    }
}
