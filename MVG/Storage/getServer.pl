...
getServer - Collect Server information for Customer

=head1 VERSION HISTORY

version 1.0 21 October 2009 DV
...
This application will get the Server information related to the shared storage from the customer.

=head1 SYNOPSIS

 getServer.pl [-t] [-l log_dir] 

 getServer -h	 	   Usage
 getServer -h 1	   Usage and description of the options
 getServer -h 2	   All documentation
...
my ($logdir, $dbhsource, $dbhtarget, @servergroups);
my $username = "root";
...
sub add2server {
	my($servergroup,$server,$os,$model,$serial,$rack,$enclosure,$role,$assetCode) = @_;
	my $query = "INSERT INTO server (servergroup,server,os,model,serial,rack,enclosure,role,assetCode) 
				 VALUES ('$servergroup','$server','$os','$model','$serial','$rack','$enclosure','$role','$assetCode')";
...
my $query = "SELECT distinct(serverg) FROM t_serverg WHERE scope='eib'";
...
while (my $ref = $sth->fetchrow_hashref) {
	...
	push @servergroups, $serverg;
}
$sth->finish();

foreach my $serverg (@servergroups) {
	my $query = "SELECT server,os,model,serial,rack,enclosure,role,asset_cd FROM t_server WHERE serverg='$serverg'";
	my $sth = $dbhsource->prepare($query);
	my $rv = $sth->execute();
	if (not(defined $rv)) {
		error("Could not execute query $query, Error: ".$sth->errstr);
	...
	while (my $ref = $sth->fetchrow_hashref) {
		...
		my $os = $ref->{os} || "";
		my $model = $ref->{model} || "";
		my $serial = $ref->{serial} || "";
		my $rack = $ref->{rack} || "";
		my $enclosure = $ref->{enclosure} || "";
		my $role = $ref->{role} || "";
		my $asset_cd = $ref->{asset_cd} || "";
		add2server($serverg,$server,$os,$model,$serial,$rack,$enclosure,$role,$asset_cd);
	}
	$sth->finish();
}
