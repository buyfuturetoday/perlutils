...
LargeFiles - Extract the top-20 large files per drive.
...
This application will extract the top-Z0 large files per drive.

=head1 SYNOPSIS

 LargeFiles.pl [-t] [-l log_dir] [-a | -n YYYY-MM-DD]

 LargeFiles -h		   Usage
 LargeFiles -h 1	   Usage and description of the options
 LargeFiles -h 2	   All documentation
...
sub handle_query($) {
	...
	print "$datetime - Handling query for $server, drive $drive\n";
	my $query = "SELECT filename, path, sizeusedmb, allocatedmb, created, modified, accessed, createddays, modifieddays, accesseddays, filetype FROM $tablename ORDER BY sizeusedmb DESC LIMIT $limit";
	logging($query);
	...
	while (my $ref = $sth->fetchrow_hashref) {
		my $filename = $dbh->quote($ref->{filename});
		my $path = $dbh->quote($ref->{path});
		...
		my $filetype = $dbh->quote($ref->{filetype});
		add2resulttable($filename, $path, $sizeusedmb, $allocatedmb, $created, $modified, $accessed, $createddays, $modifieddays, $accesseddays, $filetype, $server, $drive);
	}
	$sth->finish();
}
...
sub add2resulttable() {
	my($filename, $path, $sizeusedmb, $allocatedmb, $created, $modified, $accessed, $createddays, $modifieddays, $accesseddays, $filetype, $server, $drive) = @_;
	my $query = "INSERT INTO $resulttable 
				 (`filename`, `path`, `sizeusedmb`, `allocatedmb`, `created`, `modified`, `accessed`, `createddays`, `modifieddays`, `accesseddays`, `filetype`, `server`, `drive`) VALUES
				 ($filename, $path, '$sizeusedmb', '$allocatedmb', '$created', '$modified', '$accessed', '$createddays', '$modifieddays', '$accesseddays', $filetype, '$server', '$drive')";
