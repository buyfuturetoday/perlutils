...
orphanBookPages - Check for orphan Book pages

=head1 VERSION HISTORY

version 1.0 19 March 2010 DV
...
This application will check for orphan book pages.

=head1 SYNOPSIS

 orphanBookPages.pl [-t] [-l log_dir] [-d databasename]

 orphanBookPages -h	   Usage
 orphanBookPages -h 1   Usage and description of the options
 orphanBookPages -h 2   All documentation
...
use DBI();
use Log;
use storParams;			# DB Connection parameters
# use File::Basename;
...
# Find All Orphan Book Pages
my $query = "SELECT nid, title, type
				FROM node
				WHERE nid not in (select distinct nid from book)";
...
while (my $ref = $sth->fetchrow_hashref()) {
	...
	my $title = $ref->{title};
	my $type = $ref->{type};
	error("$nid - $title orphan page ($type)");
}
$sth->finish();
