...
loadLog.pl - This script will load Unix agent template file on agent systems.
...
The agent template is the (modified) caiLogA2.config file that was created with the saveconfigs.pl script or the mkconfig script.

=head1 SYNOPSIS

 loadLog.pl [-t] [-l logfile_directory]  [-u username] [-p password] [-s hosts-file | -i host] [-c command-file] -a agent-template

 loadLog.pl -h	    Usage Information
 loadLog.pl -h 1    Usage Information and Options description
 loadLog.pl -h 2    Full documentation
...
File containing all commands to be executed. Each command must log output to the file log.txt in the default directory. Default: loadLogA2.txt
