...
WRSExtractGroup.pl - Script to extract Group data from the CAUM4WS table for all transactions.
...
The script will connect to the CAUM4WS database and extract data. The data will be made available in a html file on the reporting directory. The data from one identifier with the last x connections will be extracted.

=head1 SYNOPSIS

 WRSExtractDevFailed.pl [-t] [-l log_dir] [-d datasource] -i instancenumber

    WRSExtractDevFailed.pl -h	  Usage
    WRSExtractDevFailed.pl -h 1    Usage and description of the options
    WRSExtractDevFailed.pl -h 2    All documentation
...
DeviceID number from the WRMGroup table.
...
# Open Output file
my $outfile = $rep_dir."/group_$identifier.html";
...
print OutFile "<tr bgcolor='yellow'><th>Group<th>Id.<th>Time<th>MonStatus<th>Resp.(ms)";
