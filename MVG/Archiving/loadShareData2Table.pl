...
loadShareData2Table-bytes - Load File Share data to table. SizeUsed and SizeAllocated in bytes.
...
SizeUsed and SizeAllocated are in bytes, reprocessing will be required.
...
my $expectedfieldstring = "FileName,Path,SizeUsedBytes,AllocatedBytes,Created,Modified,Accessed,CreatedDays,ModifiedDays,AccessedDays,FileType";
my $unicode = "UCS-2BE";
my $multimax = 1000;
...
while (my $line = decode($unicode, <Filedata>)) {
	...
	if ((substr($line,0,length($delim)) eq $delim) ||
		...
		(substr($line,0,length($delim2)) eq $delim2)) {
		logging("OK, found the Object name $line");
...
while (my $line = decode($unicode, <Filedata>)) {
	...
	if (substr($line,0,length($delim)) eq $delim) {
		...
		if ($fieldstring eq $expectedfieldstring) {
			...
			error("Not the expected field string ***$fieldstring*** (expected ***$expectedfieldstring***)");
			# exit_application(1);
			last;
		}
	}
